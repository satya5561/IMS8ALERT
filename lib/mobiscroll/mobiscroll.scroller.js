(function ($, window, document, undefined) {
    ...
    classes.Scroller = function (el, settings, inherit) {
        ...
        function addFade(val, t) {
            if (val) {
                removeFadeClass(t)
                var ix = 4;
                for (var i = 1; i <= 4; i++) {
                    var cell = $('.dw-li[data-val="' + (parseInt(val) - i) + '"]', t);

                    if (cell.length > 0) {
                        cell.addClass("f-" + ix);
                    }
                    var cell2 = $('.dw-li[data-val="' + (parseInt(val) + i) + '"]', t);
                    if (cell.length > 0) {
                        cell2.addClass("f-" + ix);
                    }
                    ix = (ix - 1);
        ...
        function removeFadeClass(t) {
            $('.f-1,.f-2,.f-3,.f-4', t).removeClass("f-1 f-2 f-3 f-4");
        }
        ...
        that._generateContent = function () {
            ...
            $.each(s.wheels, function (i, wg) { // Wheel groups
                html += '<div class="mbsc-w-p dwc' + (s.mode != 'scroller' ? ' dwpm' : ' dwsc') + (s.showLabel ? '' : ' dwhl') + '">' +
                            '<div class="dwwc"' + (s.maxWidth ? '' : ' style="width:100%;"') + '>' +
                ...
                $.each(wg, function (j, w) { // Wheels
                    ...
                    html += '<' + (hasFlex ? 'div' : 'td') + ' class="dwfl"' + ' style="' +
                                ...
                                '<div tabindex="0" aria-live="off" aria-label="' + lbl + '" role="listbox" class="dwww">' +
                                    '<div class="dww" style="height:' + (s.rows * itemHeight) + 'px;margin-top:-' + (itemHeight / 2 + (s.selectedLineBorder || 0)) + 'px;">' +
                                        '<div class="dw-ul">';

                    // Create wheel values
                    html += generateWheelItems(l) +
                        '</div></div><div class="dwwo"></div></div><div class="dwwol"' +
                        (s.selectedLineHeight ? ' style="height:' + itemHeight + 'px;"' : '') + '></div></div>' +
                        (hasFlex ? '</div>' : '</td>');
